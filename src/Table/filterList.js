...
import {
 ...
 class FilterList extends React.Component {
  ...
  _sortClicked = (val) => {
    const currentSortType = val;
    let newSortType;
    if (isUndefined(currentSortType) || currentSortType === "dsc") {
    ...
    if (this.props.filterList.length > 1) {
      if (filterState) {
        ...
        this.props.filterList.map((filterItem, index) => {
           if (filterItem.visible) {
            if (this.state.searchEnabled) {
              ...
              filterListItemHtml.push(<FilterListItem key={'item_'+index} filterClicked={this._filterUpdated} index={index} label={filterItem.display} nameclass={this.state.checkBoxDisabled} selected={filterItem.selected}/>);
            }
          }
        });
        ...
        filterListHtml = (<div className={ filterListClass }>
          <p className="filtername">{this.props.filterkey}</p>
